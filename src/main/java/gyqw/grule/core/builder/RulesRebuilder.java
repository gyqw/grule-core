...
import gyqw.grule.core.model.flow.Connection;
import gyqw.grule.core.model.flow.DecisionItem;
...
import gyqw.grule.core.model.library.action.Method;
import gyqw.grule.core.model.library.action.Parameter;
import gyqw.grule.core.model.library.action.SpringBean;
import gyqw.grule.core.model.library.constant.Constant;
import gyqw.grule.core.model.library.constant.ConstantCategory;
...
public class RulesRebuilder {
    ...
    public void rebuildAction(Action action, ResourceLibrary resLibraries, Map<String, String> namedMap, boolean forDSL) {
        ...
        if (action instanceof VariableAssignAction) {
            ...
            List<gyqw.grule.core.model.rule.Parameter> parameters = methodAction.getParameters();
    ...
    private void rebuildParameters(ResourceLibrary resLibraries, List<gyqw.grule.core.model.rule.Parameter> parameters, List<Parameter> targetParameters, Map<String, String> namedMap, boolean forDSL) {
        if (parameters != null && targetParameters != null) {
            for (int i = 0; i < parameters.size(); i++) {
                if (i > targetParameters.size() - 1) {
                    break;
                }
                gyqw.grule.core.model.rule.Parameter parameter = parameters.get(i);
    ...
    private void rebuildCriteria(ResourceLibrary resLibraries, Criteria criteria, Map<String, String> namedMap, boolean forDSL) {
        ...
        if (leftPart instanceof VariableLeftPart) {
            ...
            List<gyqw.grule.core.model.rule.Parameter> parameters = part.getParameters();
            rebuildParameters(resLibraries, parameters, targetMethod.getParameters(), namedMap, forDSL);
        } else if (leftPart instanceof FunctionLeftPart) {
            FunctionLeftPart part = (FunctionLeftPart) leftPart;
            List<gyqw.grule.core.model.rule.Parameter> parameters = part.getParameters();
            if (parameters != null && parameters.size() > 0) {
                for (gyqw.grule.core.model.rule.Parameter param : parameters) {
    ...
    public void rebuildValue(Value value, ResourceLibrary resLibraries, Map<String, String> namedMap, boolean forDSL) {
        ...
        if (value instanceof ParenValue) {
            ...
            if (forDSL) {
                ...
                Constant constant = getConstantByLabel(resLibraries.getConstantCategories(), category, label);
                cv.setConstantName(constant.getName());
            } else {
                String name = cv.getConstantName();
                Constant constant = getConstantByName(resLibraries.getConstantCategories(), category, name);
            ...
            List<gyqw.grule.core.model.rule.Parameter> parameters = methodValue.getParameters();
            rebuildParameters(resLibraries, parameters, targetMethod.getParameters(), namedMap, forDSL);
        }
    ...
    private Constant getConstantByName(List<ConstantCategory> constantCategories, String category, String name) {
    ...
    private Constant getConstantByLabel(List<ConstantCategory> constantCategories, String category, String label) {
        for (ConstantCategory c : constantCategories) {
            if (!c.getLabel().equals(category)) {
                continue;
            }
